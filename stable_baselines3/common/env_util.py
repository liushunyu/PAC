...
def make_grid2op_env(
    ...
    :param wrapper_class: Additional wrapper to use on the environment.
        This can also be a function with single argument that wraps the environment in many things.
    :param wrapper_kwargs: Optional keyword argument to pass to the wrapper
    :param env_kwargs: Optional keyword argument to pass to the env constructor
    :param vec_env_cls: A custom ``VecEnv`` class constructor. Default: None.
    :param vec_env_kwargs: Keyword arguments to pass to the ``VecEnv`` class constructor.
    :param monitor_kwargs: Keyword arguments to pass to the ``Monitor`` class constructor.
    ...
    def make_env(rank):
        def _init():
            if isinstance(env_id, str):
                env = grid2op.make(env_id, **env_kwargs)
            else:
                env = env_id(**env_kwargs)
            if seed is not None:
                env.seed(seed + rank)
            ...
            # env.chronics_handler.set_filter(lambda path: re.match(".*00[0-9][0-9].*", path) is not None)
            # print(env.chronics_handler.real_data.reset())
            # env.chronics_handler.shuffle()
            # env.chronics_handler.set_chunk_size(int(24 * 60 / 5))
